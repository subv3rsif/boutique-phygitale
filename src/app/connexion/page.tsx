import { signIn, auth } from "@/lib/auth/config"
import { redirect } from "next/navigation"
import { Button } from "@/components/ui/button"
import { cn } from "@/lib/utils"

/**
 * Connexion Page - Google OAuth
 *
 * Features:
 * - Google "Sign in with Google" button
 * - Redirect to /profil after login
 * - Auto-redirect if already logged in
 * - Premium glass design (Love Symbol × Cloud Dancer)
 */

export default async function ConnexionPage() {
  // Check if user is already logged in
  const session = await auth()
  if (session?.user) {
    redirect('/profil')
  }

  return (
    <div className="flex min-h-screen items-center justify-center p-4">
      <div className="w-full max-w-md space-y-8 p-8 glass-love rounded-3xl shadow-premium-lg">
        {/* Header */}
        <div className="text-center space-y-2">
          <div className="inline-block">
            <div className="h-16 w-16 mx-auto rounded-full bg-gradient-to-br from-primary to-primary/80 flex items-center justify-center shadow-premium-purple">
              <span className="font-display font-bold text-2xl text-white">18</span>
            </div>
          </div>
          <h1 className="font-display text-4xl font-bold text-foreground">
            Connexion
          </h1>
          <p className="text-muted-foreground">
            Connectez-vous pour accéder à votre profil
          </p>
        </div>

        {/* Google Sign In Form */}
        <form
          action={async () => {
            "use server"
            await signIn("google", { redirectTo: "/profil" })
          }}
          className="space-y-4"
        >
          <Button
            type="submit"
            className={cn(
              "w-full h-12",
              "bg-white hover:bg-gray-50",
              "text-gray-700 font-medium",
              "border border-gray-300",
              "shadow-sm hover:shadow-md",
              "transition-all duration-300",
              "flex items-center justify-center gap-3"
            )}
          >
            {/* Google Icon */}
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path
                fill="#4285F4"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              />
              <path
                fill="#34A853"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              />
              <path
                fill="#FBBC05"
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              />
              <path
                fill="#EA4335"
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              />
              <path fill="none" d="M1 1h22v22H1z" />
            </svg>
            Continuer avec Google
          </Button>
        </form>

        {/* Footer */}
        <p className="text-center text-xs text-muted-foreground">
          En vous connectant, vous acceptez nos{' '}
          <a href="/cgv" className="underline hover:text-foreground transition-colors">
            CGV
          </a>{' '}
          et notre{' '}
          <a href="/politique-confidentialite" className="underline hover:text-foreground transition-colors">
            politique de confidentialité
          </a>
          .
        </p>
      </div>
    </div>
  )
}
